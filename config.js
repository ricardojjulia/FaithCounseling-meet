...
var config = {
    ...
    hosts: {
        // XMPP domain.
        domain: 'meet.faithcounseling.app',

        // When using authentication, domain for guest users.
        // anonymousdomain: 'guest.meet.faithcounseling.app',

        // Domain for authenticated users. Defaults to <domain>.
        // authdomain: 'meet.faithcounseling.app',

        // Focus component domain. Defaults to focus.<domain>.
        // focus: 'focus.meet.faithcounseling.app',

        // XMPP MUC domain. FIXME: use XEP-0030 to discover it.
        muc: 'conference.' + subdomain + 'meet.faithcounseling.app',
    },

    // BOSH URL. FIXME: use XEP-0156 to discover it.
    bosh: 'https://meet.faithcounseling.app/' + subdir + 'http-bind',

    // Websocket URL (XMPP)
    websocket: 'wss://meet.faithcounseling.app/' + subdir + 'xmpp-websocket',
    ...
    */
    ...
    // ---------------------------------------------------------------------------
    // Faith Counseling — deployment-specific settings
    // ---------------------------------------------------------------------------
    ...
    // All participants must enter their display name (the JWT provides this
    // automatically when the FaithCounseling API sets context.user.name).
    requireDisplayName: true,

    // Treat the name from the JWT as read-only so counselors and clients
    // cannot impersonate each other.
    readOnlyName: true,

    // Disable the welcome page — this instance is embedded-only.
    welcomePage: {
        disabled: true,
    },

    // Lobby: clients wait until the counselor (moderator) admits them.
    lobby: {
        autoKnock: true,
        enableChat: false,
    },

    // Do not show an invitation UI — room links are generated by the API.
    disableInviteFunctions: true,

    // Do not persist room names in the browser's recent list (HIPAA hygiene).
    doNotStoreRoom: true,

    // Disable deep-linking to mobile apps — sessions are browser-only.
    deeplinking: {
        disabled: true,
    },

    // Recording and live-streaming are off by default. Enabling them requires
    // explicit consent workflows (see PLANS/FULL-SECURITY-AND-AUDITING.md).
    recordingService: {
        enabled: false,
        sharingEnabled: false,
    },
    ...
    // Disable third-party external requests (Gravatar, analytics, etc.).
    disableThirdPartyRequests: true,
    gravatar: {
        disabled: true,
    },

    // Disable reactions and polls — not appropriate for a clinical context.
    disableReactions: true,
    disablePolls: true,

    // Whiteboard is disabled; there is no excalidraw-backend in this deployment.
    whiteboard: {
        enabled: false,
    },

    // Show only the toolbar buttons that are relevant for telehealth sessions.
    toolbarButtons: [
        'camera',
        'chat',
        'desktop',
    ...
    // Pre-join screen: let participants check devices before entering.
    prejoinConfig: {
        enabled: true,
    ...
    // Application logo shown on the pre-join and in-call header.
    defaultLogoUrl: 'images/watermark.svg',

    // Hide the moderator indicator badge (counselor role is internal).
    disableModeratorIndicator: false,

    // STUN servers used for P2P fallback (two-participant counseling calls).
    p2p: {
        enabled: true,
        stunServers: [
            { urls: 'stun:meet.faithcounseling.app:3478' },
        ],
    },

    // Disable analytics by default to avoid sending session data externally.
    analytics: {
        disabled: true,
    },

    // Legal URLs for Faith Counseling.
    legalUrls: {
        helpCentre: 'https://faithcounseling.app/support',
        privacy: 'https://faithcounseling.app/privacy',
        terms: 'https://faithcounseling.app/terms',
    },
};
